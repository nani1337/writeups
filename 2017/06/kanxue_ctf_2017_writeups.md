* 目录
  * [0x01_windows_crackme](https://github.com/walkerfuz/writeups/master/2017/06/kanxue_ctf_2017_writeups.md#0x01_windows_crackme)
...
```
猜测该函数用于获取输入的字符串。利用OllyDbg动态调试该程序，在0040120B处下断点，F8单步执行，查看EAX返回值可以猜测是正确的。

根据IDA静态分析+Ollydbg动态调试，可以分析出上述代码的大致含义（标注在了上述代码片段中）：
> 输入的字符串长度为4且第一个字符为数字1，第二个字符为数字5，第三和第4个字符需满足以下公式：
> `(x-0.2) * y * 16 = 384.00`
输入的字符占1个字节，即0x0-0xFF之间，直接利用循环爆破：
```C
...
```
对比可以发现00401262处的E8并没有被调用，OD正确地将00401262和00401263区分为两条指令，IDA却没有正确解析。同样，之后004012AA处也是这种情况：
IDA：
```assembly
.text:004012A6                 jz      short near ptr loc_4012AA+1
.text:004012A8                 jnz     short near ptr loc_4012AA+1
...
```
这两个地方的错误解析，也导致了IDA无法正确实现F5功能，直接在代码片段中F5，会提示Please position the cursor within a function警告。如果右击-->create function会提示：
```bash
...
解决办法：将00401262和004012AA处的E8修改为NOP指令，得到patch后的exe得到反编译后的代码：
```C
int sub_4011F4()
{
  double v0; // st7@8
...
```
得到反编译后的类C代码，可以直接得到计算公式，比汇编清爽不少。也可以在不修改exe的情况下，用鼠标选择004011F4至004012E8的所有代码，按住快捷键P（create function），然后再按F5即可得到反编译代码。但这是一种取巧的方法：
```
...
总结：此题是入门题，没有任何反调试手段，主要考验分析crackme的基本功力，涉及知识点包括：
1. 浮点数的运算与存储
2. IDA反编译功能F5的修正
